...
class MarcIndexer < Blacklight::Marc::Indexer
  ...
  def initialize
    ...
    to_field 'marc_ssim', get_xml
    to_field "text_tesim", extract_all_marc_values do |r, acc|
      acc.replace [acc.join(' ')] # turn it into a single string
    end
     
    to_field "language_ssim", marc_languages("008[35-37]:041a:041d:")
    to_field "format_ssim", get_format
    ...
    to_field 'material_type_ssim', extract_marc('300a', :trim_punctuation => true)
     
    # Title fields
    #    primary title 
     
    to_field 'title_tesim', extract_marc('245a')
    to_field 'title_ssim', extract_marc('245a', :trim_punctuation => true, :alternate_script=>false)
    to_field 'title_vern_ssim', extract_marc('245a', :trim_punctuation => true, :alternate_script=>:only)
     
    #    subtitle
     
    to_field 'subtitle_tesim', extract_marc('245b')
    to_field 'subtitle_ssim', extract_marc('245b', :trim_punctuation => true, :alternate_script=>false)
    to_field 'subtitle_vern_ssim', extract_marc('245b', :trim_punctuation => true, :alternate_script=>:only)
     
    #    additional title fields
    to_field 'title_addl_tesim', 
      extract_marc(%W{
        245abnps
        130#{ATOZ}
        240abcdefgklmnopqrs
    ...
    to_field 'title_series_tesim', extract_marc("440anpv:490av")
     
    to_field 'title_ssi', marc_sortable_title  
    ...
    to_field 'author_ssim', extract_marc("100abcdq:110#{ATOZ}:111#{ATOZ}", :alternate_script=>false)
    ...
    to_field 'author_ssi', marc_sortable_author
     
    # Subject fields
    to_field 'subject_tesim', extract_marc(%W(
      600#{ATOU}
      610#{ATOU}
      611#{ATOU}
      630#{ATOU}
    ...
    to_field 'subject_addl_tesim', extract_marc("600vwxyz:610vwxyz:611vwxyz:630vwxyz:650vwxyz:651vwxyz:654vwxyz:655vwxyz")
    ...
    # Publication fields
    to_field 'published_ssim', extract_marc('260a', :trim_punctuation => true, :alternate_script=>false)
    to_field 'published_vern_ssim', extract_marc('260a', :trim_punctuation => true, :alternate_script=>:only)
    to_field 'pub_date_ssim', marc_publication_date
     
    # Call Number fields
    to_field 'lc_callnum_ssim', extract_marc('050ab', :first => true)
    ...
    to_field 'lc_b4cutter_ssim', extract_marc('050a', :first=>true)
     
    # URL Fields
     
    notfulltext = /abstract|description|sample text|table of contents|/i
     
    to_field('url_fulltext_ssim') do |rec, acc|
    ...
    # Very similar to url_fulltext_ssim. Should DRY up.
    to_field 'url_suppl_ssim' do |rec, acc|
      rec.fields('856').each do |f|
        case f.indicator2
        when '2'
          f.find_all{|sf| sf.code == 'u'}.each do |url|
